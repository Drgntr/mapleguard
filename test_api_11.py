    ...
    try:
        ...
        if "characters" in data:
            ...
            if len(classes) == 1:
                ...
                out.append(f"{name:20}: IGNORED. Got: {str(classes)[:50]}")
            else:
...
async def main():
    ...
    variations = [
        ("categoryNo prefix", {**base_filter, "categoryNo": 1020201}), 
        ("categoryNo full", {**base_filter, "categoryNo": 1020201001}), 
        ("class WARRIOR", {**base_filter, "class": "WARRIOR"}), 
        ("class Class_WARRIOR", {**base_filter, "class": "Class_WARRIOR"}), 
        ("job WARRIOR", {**base_filter, "job": "WARRIOR"}), 
        ("className WARRIOR", {**base_filter, "className": "WARRIOR"}),
    ]
    ...
    with open("test_api_11_out.txt", "w", encoding="utf-8") as file:
        file.write("\n".join(out_file))
